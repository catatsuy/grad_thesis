...
This implementation is more efficient if the \textit{reg} class instance is not updated very frequently.
...
The implementation of \figref{src:reg} is removed of the variable \textit{set\_}.
...
solid arrows represent the assignment.
...
If you specify the \textit{dynamic} as a way to schedule generally,
there are benefits that load assigned to each thread is almost equal,
but there are disadvantages large overhead.
