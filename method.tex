...
\section{高速化の方針}
\fi
\section{A Direction for High-speed Simulation}

\if0
高速化の方針として逐次プログラミングにおける最適化と並列化の両方を考える．
\fi
As a direction for high-speed simulation,
I propose both optimizations in sequential programming and parallelization.

\if0
\section{逐次プログラムにおける高速化手法}
\fi
\section{Proposed methods with sequential program}
...
\fi
In the implementation shown in \figref{src:reg},
ArchHDL gives non-blocking assignment and blocking assignment as methods for updating the value of the \textit{reg} class instance.

\if0
ブロッキング代入について考える．reg クラスのインスタンスにブロッキング代入が行われた時に curr\_ の値を書き換える．
\fi
As to blocking assignment,
...
\fi
On the other hand, as to non-blocking assignment,
the member variable \textit{set\_} of the \textit{reg} class instance is copied to true
...
Only when the member variable \textit{set\_} is true, the member variable \textit{next\_} is copied to the value of the member variable \textit{curr\_} in the \textit{Update} method, which is a member method of \textit{reg} class.
It shows the value of the register is updated to the new value before the cycle in which the non-blocking assignment is carried out to the \textit{reg} class instance.
...
Therefore the unnecessary assignment is avoided by using the variable \textit{set\_}.
...
\fi
 \caption{The source code of \textit{reg} class which is removed the conditional branch}
...
It is removed the variable \textit{set\_} from the implementation shown in \figref{src:reg}.
...
and the \textit{Update} method of the \textit{reg} class instances is called.
...
\fi
As to the implementation of the proposed method,
...
but the next collections are copied to the curr collections.
...
\fi
I have proposed the methods with sequential programming.
I propose the parallelization of the execution of multiple instances in this section.
...
\fi
As shown from 40 to 47 lines in \figref{src:class_singleton},
...
Its proof is the research task from now on.
...
\figref{src:exec_openmp} shows parallelized code of the line from 40 to 47 in \figref{src:class_singleton}.
The number of threads can be changed by the environment.
The line 2 of \figref{src:exec_openmp} shows parallelization in 8 threads.
...
it has the advantage that the tasks are assigned to each thread almost equally.
But It has the disadvantage of large overhead.
If each of the tasks of each register and each module is not very different from the hardware description with ArchHDL,
